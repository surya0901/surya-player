...
export async function connectSpotify() {
  ...
  const params = new URLSearchParams({ client_id: clientId, response_type: 'code', redirect_uri: redirect, scope: 'playlist-read-private playlist-read-collaborative', code_challenge_method: 'S256', code_challenge: challenge, state });
...
export async function spotifyPlaylists() {
  ...
  const token = stored.expires > Date.now() + 60000 ? stored.access_token : await exchange({ grant_type: 'refresh_token', refresh_token: stored.refresh_token });
  const playlists = [];
  ...
  while (url) {
    ...
    if (!res.ok) throw new Error('Could not load Spotify playlists. Please try again.');
    const data = await res.json();
